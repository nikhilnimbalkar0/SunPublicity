  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const categoryDocRef = doc(firestore, "categories", "Wall Paintings");
        const colRef = collection(categoryDocRef, "hoardings");
        const snapshot = await getDocs(colRef);
        const list = snapshot.docs.map((doc) => normalizeHoardingData(doc.data(), doc.id, "Wall Paintings"));

        setItems(list);
      } catch (err) {
        setError(err.message || "Failed to load corporate hoardings");
        setItems([]);
      } finally {
        setLoading(false);
      }
    ...
    <>
      ...
      <main className="min-h-screen p-6 md:p-10 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6">
          <h1 className="text-2xl md:text-3xl font-bold">Wall Paintings</h1>
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading && (
            <div className="col-span-full text-center text-gray-500 py-10">Loading corporate hoardings...</div>
          )}
          {!loading && error && (
            ...
            >
              <div className="aspect-video bg-gray-100 overflow-hidden relative">
                <img
                  ...
                  onError={(e) => {
                    e.currentTarget.src = "https://placehold.co/800x450?text=Wall+Paintings";
                  }}
                />
                <span
              ...
              <div className="p-5 space-y-3">
                ...
                <div className="pt-2 flex flex-col gap-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => navigate('/booking', { state: { item: { id: b.id, location: b.location, size: b.size, price: b.price, image: b.image, href: `/wall-paintings/${b.id}` } } })}
                    ...
                    {(() => {
                      ...
                      return (
                        <button
                          onClick={() => toggle({ id: b.id, location: b.location, size: b.size, price: b.price, image: b.image, href: `/wall-paintings/${b.id}` })}
          ...
          {!loading && !error && filtered.length === 0 && (
            <div className="col-span-full text-center text-gray-500 py-10">No corporate hoardings found.</div>
          )}
        </section>
      </main>
    </>
